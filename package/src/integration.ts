...
export const integration = defineIntegration({
	...
	setup(context) {
		type ModernizrBuilder = {
			build: (options: typeof context.options, callback: (result: string) => void) => typeof modernizr
		};
		
		return {
			hooks: {
				'astro:config:setup': ({injectScript}) => {
					(context.options as any)["feature-detects"] = context.options.featureDetects;
					context.options.minify ??= true;
					(modernizr as unknown as ModernizrBuilder).build(context.options, (result: string) => {
